...
kernel. The fact that \enquote{behind the scenes} they are built on \pkg{expl3}
...
\begin{itemize}[font=\ttfamily]
  \item[m] A standard mandatory argument, which can either be a single token
...
\begin{itemize}[font=\ttfamily]
  ...
  \item[d] Given as \texttt{d}\meta{token1}\meta{token2}, an optional
    ...
    returned.
  \item[O] Given as \texttt{O}\marg{default}, is like \texttt{o}, but returns
    \meta{default} if no value is given.
  \item[D] Given as \texttt{D}\meta{token1}\meta{token2}\marg{default}, it is
...
\begin{function}
  ...
  \begin{syntax}
    \cs{NewDocumentCommand} \meta{command} \Arg{arg spec} \Arg{code}
  \end{syntax}
  This family of commands are used to create a \meta{command}. The argument
...
\begin{function}
  ...
  create environments (\cs{begin}\Arg{environment} \ldots{}
  \cs{end}\Arg{environment}). Both the \meta{start code} and \meta{end code}
...
command. The most common example is the use of \cmd{\\} in \pkg{amsmath}
...
does not need to access the contents of the environment (its \enquote{body}),
